...
批量抽取另执行跨表环境强度派生（`env_intensity.py`）：同公司、同报告期、同集团口径下以
ESG/年报环境总量÷年报合并营业收入派生温室气体、能源、水强度（千克/万元）。分母只取年报
合并口径营业收入（中文主要会计数据摘要或英文合并损益表显式RMB单位，港币/美元拒绝）；总量
只取集团层面披露（中文显式年份表头表格行与单值自由行、英文Group锚定自由文本与显式年份
表头行），拒绝范围一/二单口径、直接/间接分段、循环水、目标/约/增加/减少措辞、宏观叙事、
equity basis及产量口径。温室气体总量须过范围闭环核验：未显式标注范围一+二时同页范围三
正值即拒绝，范围一/二行可解析时要求总量≈范围一+范围二（1%容差）。公司已按收入口径披露
强度、已有候选、总量不一致或营收不一致时抑制派生，派生值须过量级防护区间。派生观测以
`中文跨表派生: `/`English cross-document derived: `严格前缀进入自动策略。

总量布局另含公司主体锚定的叙述式句中总量（如“2025 年，公司温室气体排放总量5532.27吨”），
数值须止于句末或句读，`降至/控制在/以内`等目标措辞一律拒绝；GHG总量兼容`百万吨二氧化碳
当量`单位与空格范围标注（`（范围 1+ 范围 2）`），范围一/二行标签兼容数字写法与可省“量”。
中文环境表直接抽取同时支持`披露项 单位`年份表头与`万元营收/万元营业收入`强度单位：公司
自披露收入口径强度优先于派生并使派生抑制，不与派生值制造冲突组。

跨表派生已推广至SO₂、NOx、PM、废水、一般固废、危废六指标（SO₂/NOx/PM方法论口径为
克/万元，其余为千克/万元）：总量句型新增陈述式（标签+数值+单位+；;，,。、终止符，不锚
行首），叙述/陈述式以整句防护窗口（句末标点为界、上限120字符，PDF换行不是句边界）拦截
`目标/计划/减少`等措辞与`N家火电企业/所属/子公司`部分口径锚点；`减排标准/减排任务`等
复合名词不构成目标措辞。同页年列表定位值优先于单值/叙述/陈述值，不一致残片（上年列
断行）剔除而非制造假冲突。抑制规则：`万元`后接`产值`不匹配（产值口径不抑制）、`每百万
营收`与`百万营收`内的`营收`不匹配（防跨行桥接）、`单位营收/每百万营收`前缀披露直接
抑制；`范围三/范畴三`与`每百万营收/单位营收`前缀的温室气体总量一律拒绝。派生观测同样
以严格前缀进入自动策略，六指标99条派生全部自动确认。温室气体总量标签后顾另补齐
阿拉伯数字`范围1/2/3、范畴1/2/3`（拼版残块中“范围2温室气体排放总量”子项不再冒充
集团总量），范围闭环核验的范围一/二行标签同步兼容阿拉伯数字。

同一跨表派生纪律已推广到三项社会投入占比指标（`social_invest.py`）：以报告期环保投入、
安全生产投入、对外捐赠总额÷年报合并营业收入派生占营收比例。总额版式覆盖营业外支出
附注行（“本期发生额/上期发生额”表头锚定首列，单位须本页“单位：元/万元”明示，胶粘行
以首值=第三值为结构锚点）、年份表头行、单值行、值先式（ESG亮点块）、公司锚定叙述式
与陈述式；累计/历年口径、预算/计划/拟口径、强度/密度口径、接受捐赠及税会差异
（捐赠支出超出标准部分）一律拒绝。公司已披露占营收比例时抑制派生；年报附注与ESG
披露总额不一致或营收不一致时放弃，不制造口径冲突组；派生值须过合理区间（环保
0.001—50%、安全0.0001—30%、捐赠0.00001—10%）。派生观测以`中文投入占比派生: `严格
前缀进入自动策略，332条派生全部自动确认。

温室气体减排率另以`derive_ghg_reduction_candidates`从同一显式年份表头总量行的本期/
上期两值派生（上期-本期）/上期×100%，两值同行同单位口径闭环天然成立，负值（排放
上升）按英文规则同域[-1000,100]保留；范围一/二行两期值均可解析时本期与上期均须满足
总量≈范围一+范围二（1%容差），否则回退本期标准范围核验。页面附注明示“仅统计…集团
本部/不含子公司”时两期不可比整页拒绝；公司直接披露同口径减排率/同比下降时抑制派生，
目标/计划措辞不构成披露。派生观测以`中文两期总量表派生: `严格前缀进入自动策略，
14条派生全部自动确认，减排率人口7→21。

公司直接披露的同口径减排率由`_extract_chinese_ghg_reduction_direct`严格抽取：仅接受
单年表头KPI表“温室气体排放同比下降 %”行与公司锚定“同比减少/下降X%”叙述（须报告年/
报告期内锚定）两种版式，目标/峰值/较基准年/累计/强度/净排放一律拒绝；与派生互斥不
混用，直接披露公司不再派生。候选以`中文减排率直接披露: `严格前缀进入自动策略，
人口21→23。
...
旧版评分器对没有确认值的指标计0分，这会把披露完整度与实际表现混在一起，只允许保留为
`legacy_zero_v1`开发对照策略。后续评分必须显式选择版本化缺失策略，并至少支持保守0分、
指标中性分和已披露权重重分配三种情景。研究版同时输出敏感性；正式版只能使用客户确认并冻结的
一种策略，不能修改历史批次。
...
- `research`：全自动预排名，只使用系统策略允许的自动决定，输出覆盖率、可信等级、缺失情景和
  排名稳定性，并明确标注非正式；
- `release`：使用客户正式方法论，只接收正式策略允许的自动确认及人工确认数据，通过公司池、
  审核和冻结门禁后发布。

正式模式必须通过`--missing-strategy`显式指定已冻结策略，禁止使用隐藏默认值；输入包含任何
`pending`观测时直接拒绝。研究模式元数据记录算法版本、输入和方法论SHA-256，并输出
`ranking_sensitivity.json`。

建议使用独立版本号，例如`auto_prerank_v1`和`client_formal_2026_v1`。两者共享原始证据，不能
共享未标明来源的评分决定。每条观测须记录决定类型、置信度、适用算法版本及是否允许正式发布。

## 定性指标

自动文本匹配在正式方法论中默认只产生建议档位；在自动预排名中可由版本化规则生成机器分档，
但必须同时保存正证据、反证据、判断理由和置信度。复核员依据报告标准选择：
...
正式方法论的建议档位最高为80分；100分必须由复核员依据完整性、效果和行业领先证据主动判定。
自动预排名可以输出100分建议，但必须标为机器判断且不得进入正式观测。正式复核包中的建议档位、
质量等级及优先级都不改变`pending`状态。
...
人工审核采用风险预算，不要求无差别逐格签名：主体身份、实质冲突、定性80/100、高权重弱证据和
排名敏感项必须审核；严格公式派生及一致多来源采用分层抽样，抽样失败才扩大范围。系统应记录自动
决定比例、抽样错误率、审核一致率和仍可能改变前200的开放任务数。

定量人工队列可按排名影响重排：

```bash
PYTHONPATH=src python3 -m aegis_esg.cli prioritize-review-impact \
  output/audit/all_markets_candidate_review_tiers_2025.csv \
  output/research/2025/ranking_sensitivity.json --top-n 200 \
  --output output/audit/all_markets_rank_impact_review_2025.csv \
  --summary output/audit/all_markets_rank_impact_review_summary_2025.json
```

`rank-impact-v1`综合冲突层级、前N边界距离、三策略名次跨度、指标权重和置信度风险，同时保留按
置信度及按权重排序的基线名次。该队列只调度审核，不自动选择候选值或产生签名决定。

候选可构建为稳定的多源证据约束图：

```bash
PYTHONPATH=src python3 -m aegis_esg.cli build-evidence-graph \
  data/review/all_markets_indicator_candidates_2025.csv \
  --output output/audit/all_markets_evidence_constraint_graph_2025.json \
  --summary output/audit/all_markets_evidence_constraint_graph_summary_2025.json
```
