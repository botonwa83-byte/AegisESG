...
废水1家、危废4家，SO₂和颗粒物暂无合规命中。替代水资源利用率仅接受0%—100%的明确再生水
占比，拒绝循环水利用率，`03337.HK`两页一致披露8.53%。清洁能源强度仅接受标准煤/人民币营收，
真实样本无合规命中；环保投入占比命中`01250.HK`的0.18%。公益捐赠占比仅接受明确营收比例，
...
人民币附注和“本期增加额”表头闭环时派生，命中`01713.HK`。当前生成413条候选，覆盖16项治理、
9项环境和8项社会指标；`00600.HK`附录PDF列顺序、
...
当前矩阵为92×37=3,404项：400项已有待审候选，3,004项仍需扩展抽取规则，其中863项为关键
指标缺口。任务按“缺失关键指标、已有关键候选、缺失普通指标、已有普通候选”排序；矩阵
`applicable=false`，不会把候选误作确认数据。摘要另显式输出零覆盖指标数量和代码，当前仅余
清洁能源强度、SO₂强度、颗粒物强度和销售现金实现率4项；它们均已有严格规则但现有报告无合规命中。
...
`public-disclosure-v4`对新增指标同时校验指标代码、官方报告类型、稳定证据前缀和置信度。当前400组
候选分为394组可自动确认、3组冲突强制人工签名、1组一致多候选抽查和2组单候选抽查。该计划
`applicable=false`，只描述下一动作。`resolve-pending`预览生成394条确认、11条未决候选和400条
