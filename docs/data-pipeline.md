...
英文合并损益表还支持以`Revenue`当年/上年值派生营业收入增长率；比例计算不涉及币种换算。报表
项目必须恰好得到两期数值，或“1个小整数附注号+2期数值”；单期缺失、污染多列和疑似附注号加
单值均拒绝。当前生成82条候选：资产负债率30家、收入增长率52家，无重复或冲突。所有观测保持
`pending`，须经现有解析器或签名复核确认后才可进入评分数据；负净资产案例不自动截断。
