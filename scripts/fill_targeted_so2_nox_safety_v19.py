#!/usr/bin/env python3
"""Targeted research fill for SO2/NOx/safety recall (v19 → v42).

Only re-extracts companies missing those key indicators that also have
text-export clue hits for the new label/unit patterns. Research-only.
Does not map 硫氧化物/SOx to SO2.
"""
...
BASE_OBS = ROOT / "output/research/2025/full_auto_observations_v41_enriched.csv"
...
OUT_OBS = ROOT / "output/research/2025/full_auto_observations_v42_enriched.csv"
AUDIT_JSON = ROOT / "output/audit/authority_gap_fill_v19_2025.json"
AUDIT_CSV = ROOT / "output/audit/authority_gap_fill_v19_2025.csv"
TAG = "[research-only:authority-gap-fill-v19;not-formal]"
TARGET = {"Q_E_SO2_INTENSITY", "Q_E_NOX_INTENSITY", "Q_S_SAFETY_INVEST_RATE"}

CLUE = re.compile(
    r"(?:"
    r"安全生产投入占营业收入(?:的)?比例\s*[%％]|"
    r"氮氧化物\s*[（(]\s*NO[xXₓ]\s*[）)]\s*排放强度|"
    r"二氧化硫\s*[（(]\s*SO[₂2]\s*[）)]\s*排放量|"
    r"氮氧化物排放量\s*[（(]\s*NO[xXₓ]\s*[）)]|"
    r"二氧化硫(?:\s*[（(]\s*SO[₂2]\s*[）)])?\s*排放强度\s*吨\s*[／/]\s*百万元|"
    r"氮氧化物(?:\s*[（(]\s*NO[xXₓ]\s*[）)])?\s*排放强度\s*吨\s*[／/]\s*百万元"
    r")",
    re.I,
)
...
def _quality(item: Observation) -> tuple[int, float]:
    ...
    # Hard reject SOx→SO2 pollution if any slipped through.
    if item.indicator_code == "Q_E_SO2_INTENSITY" and re.search(
        r"硫氧化物|硫化物|SOx", evidence,
    ) and not re.search(r"二氧化硫|SO2|SO₂", evidence):
        return 99, 0.0
    rank = 3
    if "Chinese" in evidence and "table row" in evidence:
        rank = 0
    elif "中文跨表派生" in evidence or "English revenue intensity" in evidence:
        rank = 1
    elif "中文投入占比派生" in evidence:
        rank = 1
    elif re.search(r"安全生产投入占营业收入", evidence) and "Chinese" not in evidence:
        rank = 2
...
def main() -> None:
    ...
    for code in names:
        for indicator in TARGET:
            if present.get((code, indicator)) is None:
                gap_codes.add(code)
    ...
    target_codes = sorted(code for code in gap_codes if _has_clue(by_company.get(code, [])))
    ...
    summary = {
        ...
        "notice": "仅对SO2/NOx/安全投入缺数且含新规则线索的公司定点召回；不把硫氧化物等同SO2；非正式。",
    }
