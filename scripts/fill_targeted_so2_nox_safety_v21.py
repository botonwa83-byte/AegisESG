#!/usr/bin/env python3
"""Targeted research fill for SO2/NOx/safety recall (v21 → v44).

Clue-filtered re-extract for companies missing key intensities/rates.
Focus: English SO2/NOx totals with Ton/(tonnes) units and multiline Emissions
rows; prior v20 safety/NOx clues retained. Research-only.
...
OUT_OBS = ROOT / "output/research/2025/full_auto_observations_v44_enriched.csv"
AUDIT_JSON = ROOT / "output/audit/authority_gap_fill_v21_2025.json"
AUDIT_CSV = ROOT / "output/audit/authority_gap_fill_v21_2025.csv"
TAG = "[research-only:authority-gap-fill-v21;not-formal]"
...
def main() -> None:
    ...
    summary = {
        "policy_version": "authority-gap-fill-v21-targeted-so2-nox-safety",
        ...
        "notice": "英文SO2/NOx Ton/(tonnes)多行总量派生；邻行per-barrel强度不抑制；不把硫氧化物等同SO2；不换算港币营收；非正式。",
    }
