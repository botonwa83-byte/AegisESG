...
*{{box-sizing:border-box}}body{{margin:0;background:var(--bg);color:var(--ink);font:15px/1.65 -apple-system,BlinkMacSystemFont,"PingFang SC","Microsoft YaHei","Segoe UI",sans-serif}}
...
section{{margin-top:18px;padding:20px;overflow:auto}}table{{width:100%;border-collapse:collapse;table-layout:auto}}th,td{{padding:9px 10px;border-bottom:1px solid var(--line);text-align:left;vertical-align:top;white-space:normal;word-break:break-word}}th{{color:var(--muted);white-space:nowrap}}td code,.mono{{white-space:nowrap}}code{{color:#096b50}}article{{border-top:1px solid var(--line);padding:12px 0}}blockquote{{margin:8px 0;padding:10px 14px;background:#f5faf7;border-left:3px solid var(--accent);white-space:pre-wrap;word-break:break-word}}input{{width:min(420px,100%);padding:10px 12px;border:1px solid var(--line);border-radius:8px;font:inherit}}
...
def render_system_menu(data: dict) -> str:
    ...
    return f'''<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>aegisESP · 系统主菜单</title><style>:root{{--navy:#071a33;--navy2:#0d2a4d;--blue:#63c9ff;--muted:#9cb1c8;--line:#234667;--panel:#102d4d}}*{{box-sizing:border-box}}body{{margin:0;background:radial-gradient(circle at 78% 0%,#173f65 0,#071a33 38%,#061527 100%);color:#eff7ff;font:15px/1.65 -apple-system,BlinkMacSystemFont,"Segoe UI","Microsoft YaHei",sans-serif;min-height:100vh}}main{{max-width:1180px;margin:auto;padding:54px 28px 72px}}.head{{display:flex;justify-content:space-between;align-items:flex-start;gap:24px;margin-bottom:38px}}.brand{{color:var(--blue);font-weight:750;letter-spacing:1.5px;text-transform:uppercase;font-size:12px}}h1{{font-size:42px;letter-spacing:-1px;margin:12px 0 8px}}.sub{{color:var(--muted);font-size:17px;margin:0}}.state{{background:#173755cc;border:1px solid #396988;border-radius:14px;padding:15px 18px;color:#d6e8f7;text-align:right;min-width:155px;backdrop-filter:blur(8px)}}.state strong{{display:block;color:var(--blue);font-size:20px;margin:3px 0}}.state small{{color:#a9c0d5}}.menu{{display:grid;grid-template-columns:repeat(auto-fit,minmax(270px,1fr));gap:16px}}.item{{position:relative;background:linear-gradient(145deg,#123657,#0d2948);border:1px solid #285477;border-radius:17px;padding:23px;text-decoration:none;color:#eff7ff;box-shadow:0 12px 30px #0005;transition:transform .18s,border-color .18s,box-shadow .18s}}.item:hover{{border-color:var(--blue);transform:translateY(-4px);box-shadow:0 18px 36px #0008}}.item b{{display:inline-block;background:#1e5579;color:var(--blue);padding:4px 10px;border-radius:8px;font-size:12px;letter-spacing:1px}}.item h2{{margin:17px 0 5px;font-size:21px}}.item p{{margin:0;color:#a9bed2}}.item i{{position:absolute;right:21px;top:22px;color:var(--blue);font-size:28px;font-style:normal}}.metrics{{display:flex;gap:12px;flex-wrap:wrap;margin-top:30px}}.metric{{background:#102d4dcc;border:1px solid #285477;border-radius:11px;padding:11px 17px;min-width:145px}}.metric span{{display:block;color:#91aac0;font-size:12px}}.metric strong{{font-size:21px;color:#dff5ff}}</style></head><body><main><div class="head"><div><div class="brand">aegisESP · ESG科学决策系统</div><h1>系统主菜单</h1><p class="sub">从公开披露文档到 ESG 研究排名与正式发布审核</p></div><div class="state">当前工作域<strong>研究预排名</strong><small>正式发布：{"已就绪" if freeze.get("freeze_ready") else "待审核"}</small></div></div><div class="menu">{cards}</div><div class="metrics"><div class="metric"><span>研究企业</span><strong>{overview.get("company_count", "-")}</strong></div><div class="metric"><span>候选观测</span><strong>{overview.get("candidate_observation_count", "-")}</strong></div><div class="metric"><span>任务覆盖率</span><strong>{overview.get("coverage_rate", "-")}%</strong></div><div class="metric"><span>待审核冲突</span><strong>{overview.get("conflict_count", "-")}</strong></div></div></main></body></html>'''
...
# Candy-tone executive menu: defined last so both FastAPI and static export use one visual contract.
def render_system_menu(data: dict) -> str:
    overview = data["overview"]
    freeze = data.get("resolution_freeze_audit") or {}
    items = [("01", "排名中心", "一键生成研究预排名，查看覆盖率与发布状态", "/demo/ranking-center", "#ffd6e7"),
             ("02", "企业排名", "查看完整数据链企业与决策详情", "/demo/complete-chain", "#d8f5ee"),
             ("03", "数据底座", "确认原始 PDF、本地文件与 Hash 完整性", "/demo/data-readiness", "#dce8ff"),
             ("04", "审核工作台", "处理证据冲突、缺失数据与高影响任务", "/demo/review-workbench", "#fff0c7"),
             ("05", "评价方法论", "查看中文指标、E/S/G 维度、权重与规则", "/demo/methodology", "#eadcff"),
             ("06", "正式发布门禁", "检查主体、文档、审核、冻结与授权", "/demo/readiness", "#d9f0ff")]
    cards = "".join(f'<a class="item" href="{url}" style="--tint:{tint}"><span class="num">{num}</span><span class="go">↗</span><h2>{title}</h2><p>{desc}</p></a>' for num,title,desc,url,tint in items)
    return f'''<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>万绿信评 · ESG科学决策系统</title><style>:root{{--ink:#24324a;--muted:#78869a;--blue:#4e79ff;--line:#e8edf5;--bg:#f8f9fd}}*{{box-sizing:border-box}}body{{margin:0;background:radial-gradient(circle at 8% 0%,#fff1f7 0,transparent 30%),radial-gradient(circle at 95% 10%,#e6f4ff 0,transparent 30%),var(--bg);color:var(--ink);font:15px/1.65 -apple-system,BlinkMacSystemFont,"Segoe UI","Microsoft YaHei",sans-serif;min-height:100vh}}main{{max-width:1180px;margin:auto;padding:50px 28px 72px}}.top{{display:flex;justify-content:space-between;align-items:flex-start;gap:24px;margin-bottom:38px}}.brand{{color:#ef6e9b;font-size:12px;font-weight:800;letter-spacing:1.5px}}h1{{font-size:42px;letter-spacing:-1.2px;margin:11px 0 6px}}.sub{{color:var(--muted);font-size:17px;margin:0}}.state{{background:#fff;border:1px solid var(--line);border-radius:16px;padding:15px 18px;text-align:right;box-shadow:0 10px 26px #5b6c8d12}}.state span{{color:var(--muted);font-size:12px}}.state strong{{display:block;color:#ef6e9b;font-size:20px;margin:2px 0}}.state small{{color:var(--muted)}}.menu{{display:grid;grid-template-columns:repeat(auto-fit,minmax(270px,1fr));gap:17px}}.item{{position:relative;background:linear-gradient(145deg,#fff,var(--tint));border:1px solid #e4e9f3;border-radius:20px;padding:23px;color:var(--ink);text-decoration:none;box-shadow:0 12px 28px #53688714;transition:transform .18s,box-shadow .18s}}.item:hover{{transform:translateY(-5px);box-shadow:0 18px 34px #53688725}}.num{{display:inline-flex;align-items:center;justify-content:center;width:33px;height:33px;border-radius:11px;background:#fff;color:var(--blue);font-weight:800;font-size:12px;box-shadow:0 4px 10px #53688718}}.go{{position:absolute;right:22px;top:23px;color:var(--blue);font-size:21px}}.item h2{{font-size:21px;margin:18px 0 5px}}.item p{{color:var(--muted);margin:0;max-width:240px}}.metrics{{display:flex;gap:12px;flex-wrap:wrap;margin-top:29px}}.metric{{background:#fff;border:1px solid var(--line);border-radius:12px;padding:10px 17px;box-shadow:0 7px 18px #5368870d;min-width:145px}}.metric span{{display:block;color:var(--muted);font-size:12px}}.metric strong{{font-size:21px;color:#4e79ff}}.foot{{color:var(--muted);font-size:13px;margin-top:25px}}</style></head><body><main><div class="top"><div><div class="brand">万绿信评 · ESG科学决策系统</div><h1>系统主菜单</h1><p class="sub">从公开披露文档到 ESG 研究排名与正式发布审核</p></div><div class="state"><span>当前工作域</span><strong>研究预排名</strong><small>正式发布：{"已就绪" if freeze.get("freeze_ready") else "待审核"}</small></div></div><div class="menu">{cards}</div><div class="metrics"><div class="metric"><span>研究企业</span><strong>{overview.get("company_count", "-")}</strong></div><div class="metric"><span>候选观测</span><strong>{overview.get("candidate_observation_count", "-")}</strong></div><div class="metric"><span>任务覆盖率</span><strong>{overview.get("coverage_rate", "-")}%</strong></div><div class="metric"><span>待审核冲突</span><strong>{overview.get("conflict_count", "-")}</strong></div></div><p class="foot">北京万家绿色信用评级有限公司（万绿信评）品牌化研究演示 · 数据不完整时仅生成研究预排名</p></main></body></html>'''
